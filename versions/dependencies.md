# Dependency evidence across builds (0.10b → 2022)

Dependency names and versions below come from binary strings and RTTI markers. DLL-name strings are markers, not a parsed import table; missing markers do not prove absence. The 25-build sweep is a historical lead because its original package locations and hashes are not retained in the maintained catalog.


This records which third-party library markers appear in each `survarium.exe` and which versions can be read from the binary. It therefore covers symbol-less builds too. The evidence is `.rdata` version text and RTTI `.?AV…@@` type-descriptor names, neither of which requires a PDB. Method and markers: `scripts/deps_report.py`; binary acquisition: [`METHODS.md`](../METHODS.md).
...
- **Mining the selected 2022 component DLLs** by the same method finds Scaleform GFx 4.x in `vostok_scaleform.dll`, now with DefineBitsJPEG4, **libpng 1.5.27** instead of 1.5.13, and zlib 1.2.3/1.2.7 markers. The examined side-by-side files also include `bugtrap.dll`, `steam_api64.dll`, and `BEClient_x64.dll`.
- **No identifying runtime marker was found** for ODE, FreeImage, libtheora, MySQL client, minizip, Lua, or LuaJIT. Hits resembling FMOD and libcurl resolve to the libc `fmod()` name and "curly-bracket" text. This is marker evidence, not a proof that no unrecognized or dynamically resolved code exists.

## Matrix (columns = version)

| dependency \ version | 0.10b | 0.11a | 0.11b | 0.11c | 0.11e | 0.20e | 0.20f | 0.21d | 0.23h | 0.26e0 | 0.26g0 | 0.34a0 | 0.69d0 |
| --- | :-: | :-: | :-: | :-: | :-: | :-: | :-: | :-: | :-: | :-: | :-: | :-: | :-: |
...
- **Scaleform GFx** — **changes across builds**: Flash UI middleware; **not in versions.txt** (its own source snapshot is 4.0.15, but every static exe carries 4.2.21). By the 2022 build it is split into `vostok_scaleform.dll`.
  - 0.10b–0.34a0 → **4.2.21**; 0.69d0 → **in vostok_scaleform.dll**
- **zlib (engine)** — **changes across builds**: the engine's own zlib (`inflate/deflate 1.2.3 Copyright …`). Distinct from the zlib Scaleform bundles for libpng, which shows as the bare `1.2.7` atom.
  - 0.10b–0.26g0 → **1.2.3**; 0.34a0–0.69d0 → **present (no banner)**
- **OpenSSL** — **changes across builds**: TLS/crypto; the ≤0.23h builds print `OpenSSL 1.0.0g 18 Jan 2012`. **Bumped over time** — see the matrix.
  - 0.10b–0.23h → **1.0.0g (18 Jan 2012)**; 0.26e0–0.34a0 → **1.0.1h (5 Jun 2014)**; 0.69d0 → **1.1.x (static, no banner)**
...
- **STLport** — **changes across builds**: the STL implementation (`stlp_std` namespace) — Survarium does not use the MSVC STL.
  - 0.10b–0.26g0 → ✓ (5.2.1); 0.34a0–0.69d0 → —
- **Bullet**: physics/collision (`btCollisionWorld`); **not in versions.txt** but linked in every build. Its **SoftBody module** (`@btSoftBody@@`) was present 0.10b→0.26e0 and **dropped at 0.26g0** (see the 0.25–0.31 sweep).
...
- **BugTrap** — **changes across builds**: crash reporter; the exe references `BugTrap.dll` and the **4-component FileVersion read from the side-by-side `bugtrap.dll` resource** confirms 1.3.3291.42976 in every build (matches the manifest).
  - 0.10b–0.23h → **1.3.3291.42976**; 0.26e0–0.69d0 → ✓ (1.3.3291.42976)
- **SpeedTree** — **changes across builds**: vegetation renderer (`SpeedTree::` namespace + engine `speedtree_cook`). Present through 2013, then **gone from every 2014 build** — dropped at the 2013-to-2014 client restructuring (see the per-version transition evidence).
  - 0.10b–0.11e → ✓ (5.2.1); 0.20e–0.69d0 → —
- **ODE**: listed in `versions.txt` (0.11.1), but no identifying marker appears in the examined game executables; Bullet and OPCODE markers do appear.
- **FreeImage**: listed in `versions.txt` (3.12.0), but no identifying marker appears in the examined game executables.
- **libtheora**: listed in `versions.txt` (1.1.1), but no identifying runtime marker appears.
- **MySQL client**: listed in `versions.txt` (6.02), but no identifying client-executable marker appears.
...
| --- | --- | --- |
| **0.26g0** | 14 Jan 2015 | **Bullet SoftBody dropped** (`@btSoftBody@@` — present 0.10b→0.26e0, gone after) |
...
- **0.10b** (`v0.10b-build802`): `1.2.3`, `1.2.7`, `1.5.13`, `4.2.21`
- **0.11a** (`v0.11a-build816`): `1.2.3`, `1.2.7`, `1.5.13`, `4.2.21`
- **0.11b** (`v0.11b-build826`): `1.2.3`, `1.2.7`, `1.5.13`, `4.2.21`
- **0.11c** (`v0.11c-build870`): `1.2.3`, `1.2.7`, `1.5.13`, `4.2.21`
- **0.11e** (`v0.11e-build884`): `1.2.3`, `1.2.7`, `1.5.13`, `4.2.21`
